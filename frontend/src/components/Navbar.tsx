    ...
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 transition-colors">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            {isAuthenticated ? (
              <button
                ...
                className="lg:hidden p-1.5 sm:p-2 text-slate-600 dark:text-slate-300 hover:text-slate-950 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors shrink-0"
              ...
              <button
                ...
                className="md:hidden p-1.5 sm:p-2 text-slate-600 dark:text-slate-300 hover:text-slate-950 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors shrink-0"
                aria-label="Toggle navigation menu"
              >
            ...
            <div
              className={`cursor-pointer flex items-center shrink-0 ${isAuthenticated ? 'lg:hidden' : ''}`}
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            {/* Demo Tour Button - Visible on tablet/desktop, in drawer on mobile */}
            <button
              onClick={onOpenDemoTour}
              className="hidden sm:flex px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white rounded-xl text-xs font-bold shadow-xs transition-transform active:scale-95 items-center gap-1.5 cursor-pointer shrink-0"
              title="Interactive 14-Step Guided Demo for Judges"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-300 shrink-0" />
              <span>{t('nav.demo_tour')}</span>
            </button>
            ...
            {/* Language Switcher */}
            <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 sm:p-1 rounded-xl text-xs font-bold transition-colors shrink-0">
              <button
                onClick={() => setLanguage('en')}
                className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg transition-all text-[10px] sm:text-xs ${
                  language === 'en'
            ...
            {isAuthenticated && (
              <button
                ...
                className="relative p-1.5 sm:p-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer shrink-0"
                title={t('nav.notifications_tooltip')}
                aria-label={t('nav.notifications_tooltip')}
              >
                <Bell className="w-4 h-4 sm:w-5 sm:h-5" />
                {unreadCount > 0 && (
                  <span className="absolute top-1 right-1 sm:top-1.5 sm:right-1.5 w-3.5 h-3.5 bg-emerald-600 text-white rounded-full text-[9px] font-bold flex items-center justify-center animate-pulse">
                    {unreadCount}
                  </span>
                )}
              </button>
            ...
            {!isAuthenticated ? (
              <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                <button
                  ...
                  className="px-2 sm:px-3 py-1.5 text-xs font-bold text-slate-700 dark:text-slate-200 hover:text-emerald-800 dark:hover:text-emerald-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer whitespace-nowrap"
                >
                  {t('nav.login')}
                </button>
                <button
                  onClick={() => onNavigate('/register')}
                  className="hidden sm:inline-flex px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-xs transition-all cursor-pointer items-center gap-1 shrink-0"
                >
      ...
      {!isAuthenticated && publicMenuOpen && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 pt-3 pb-5 space-y-3 shadow-lg animate-in slide-in-from-top-2 duration-150">
          {/* Demo Tour Button prominently in mobile menu */}
          <button
            onClick={() => {
              setPublicMenuOpen(false);
              onOpenDemoTour();
            }}
            className="w-full py-2.5 px-3 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white rounded-xl text-xs font-bold shadow-xs flex items-center justify-center gap-2 transition-transform active:scale-[0.99]"
          >
